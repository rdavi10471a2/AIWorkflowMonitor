...
For overlay testing, refresh both `Program.cs` and `Services\GreetingCalculator.cs`, edit both matching files under `Monitor\Working\TinyConsoleWatchedProject`, then compare one file. Roslyn should validate with the sibling Working file as an overlay while still launching only one diff.
