...
For overlay testing, refresh both `Program.cs` and `Services\GreetingCalculator.cs`, edit both matching files under `Monitor\Working\<observed-root-key>\...`, then compare one file. The observed-root key includes the watched folder name plus a short path hash so same-named watched roots do not share state; the files below that key preserve paths relative to the watched root. Roslyn should validate with the sibling Working file as an overlay while still launching only one diff.
